    ...
    <div className="min-h-screen bg-neutral-50 flex flex-col max-w-lg mx-auto shadow-2xl overflow-hidden relative font-sans">
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-brand-peach/30 p-4 flex justify-between items-center shadow-sm">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => setView('menu')}>
          ...
          <div>
            <h1 className="text-xl font-bold text-brand-brown leading-none">Vó Naza</h1>
            <div className="flex items-center gap-1.5 mt-1">
              <div className={`w-2 h-2 rounded-full ${isStoreOpen ? 'bg-brand-green animate-pulse' : 'bg-red-500'}`}></div>
              <span className={`text-[9px] font-bold uppercase tracking-wider ${isStoreOpen ? 'text-brand-green' : 'text-red-500'}`}>
                {isStoreOpen ? 'Aberta Agora' : 'Fechada'}
              </span>
            </div>
          </div>
        </div>
        <button onClick={() => setIsAiOpen(true)} className="p-2 bg-brand-pink/10 text-brand-pink rounded-full flex items-center gap-1 shadow-sm active:scale-95 transition-all">
          <Sparkles size={20} />
          <span className="text-[10px] font-bold uppercase hidden sm:inline">Dúvidas?</span>
        </button>
      </header>
